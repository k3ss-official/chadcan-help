...
class ChatInterface {
    ...
    private sessionDuration: number = 15 * 60 * 1000; // 15 minutes in milliseconds
    ...
    private initEventListeners(): void {
        ...
        const mainTryButton = document.getElementById('main-try-button');

        if (tryItButton) {
            tryItButton.addEventListener('click', () => this.openChat());
        }

        if (mainTryButton) {
            mainTryButton.addEventListener('click', () => this.openChat());
        }
    ...
    private closeChat(): void {
        ...
        setTimeout(() => {
            ...
            this.chatContainer.classList.remove('chat-exit-active');
        }, 300);
    ...
    private startSessionTimer(): void {
        ...
        this.timerInterval = window.setInterval(() => {
            ...
            const elapsedTime = Date.now() - this.sessionStartTime.getTime();
            const remainingTime = Math.max(0, this.sessionDuration - elapsedTime);
